 ...
 * @author Moham
 */
public class _0_0_0_4_0_2_Attendance_ByLaw_Ajax_3_QueryPerEmploymentMastersByattendTypeDtl_AjaxServlet extends HttpServlet {
    ...
    protected synchronized void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            String wrkTypeSeq = request.getParameter("wrkTypeSeq");
            String HTML_TEXT = "";
            List<PerEmploymentMasters> pList = new PerEmploymentMasters_DAO().getEmployeesByWrkTypeCodeAndWrkTypeSeq(wrkTypeCode, wrkTypeSeq);
            if (pList.isEmpty()) {
                throw new Exception();
            }

            for (PerEmploymentMasters pem : pList) {
                HTML_TEXT += "<tr>\n"
                        + "      <td>" + pem.getEmployeeNo() + "</td>\n"
                        + "      <td>" + pem.getEmpAname() + "</td>\n";
                try {
                    HTML_TEXT += "      <td>" + pem.getPerOrganizationStructureByOrgId().getOrgName() + "</td>\n";
                } catch (Exception ex) {
                    HTML_TEXT += "      <td style='background-color: #ffd22d;'>غير مسكن على إدارة</td>\n";
                }
                HTML_TEXT += "  </tr> ";
            }
