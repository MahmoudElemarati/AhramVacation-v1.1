...
public class AppSubTab_DAO<T> extends AbstractDao<T> {
    ...
    public List<AppSubTab> getAllApplicationSubTabsAvailableToGive(String employeeId) {
        Query query = session.createQuery("SELECT p FROM AppSubTab p INNER JOIN p.appSubTabSpecAuths as f where f.perEmploymentMastersByEmployeeId.employeeId=" + employeeId + " and f.primaryAuthPass=2 ORDER BY p.subTabIndex "); //HQL
        List<AppSubTab> pList = query.getResultList();
        return pList;
    }
