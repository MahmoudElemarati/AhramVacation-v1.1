...
function createTable(data, sexFlag) {
    var header = "";
    if (sexFlag === "male") {
        header = "<th style='text-align: center;font-size: 15px;'>الإدارات</th>"
...
/////////////////////////////////////////////////////////////
    ...
    $.each(data, function (index, mainData) {
        ...
        $.each(mainData.parts, function (i, part1) {
            ...
            var total = 0;
            row = row + "<td>" + part1 + "</td>";
            $.each(mainData.gender, function (j, gender1) {
                //var vals = [];
                //Success(items_profit1.data_names);
                //names.push(gender1.gender_name);
                $.each(gender1.values, function (k, values1) {
                    if (i === k) {
                        row = row + "<td> " + values1 + "</td>";
                        total = +total + +values1;
                        if (gender1.gender_name === "ذكر") {
...
function refresh(chartType, plotType, generalCode, locationCode, sexFlag, color_1, color_2, loadtable) {
    ...
    $.ajax({
        url: "_0_0_0_1_0_3_Dash_EmpCount_Ajax_0_getdataAnalysis_AjaxServlet?"
...
function getLoad() {
    //alert(status);
    var URL = "_0_0_0_1_0_3_Dash_EmpCount_Ajax_1_getLoad_1_AjaxServlet?";
