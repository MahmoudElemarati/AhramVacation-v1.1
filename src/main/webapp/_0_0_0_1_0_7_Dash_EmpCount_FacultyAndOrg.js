...
function createTable(data, sexFlag, SearchType) {
    var header = "";
    var Title = "";
    if (SearchType === "1") {
        Title = "المؤهلات";
    ...
    if (sexFlag === "male") {
        header = "<th style='text-align: center;font-size: 15px;'>" + Title + "</th>"
...
function refresh(chartType, plotType, generalCode, locationCode, sexFlag, SearchType, QualClassCode, FacultyCode, color_1, color_2, loadtable) {
    status = 't';   
    setTheme(color_1, color_2);
    $.ajax({
        url: "_0_0_0_1_0_7_Dash_EmpCount_FacultyAndOrg_Ajax_0_getdataAnalysis_AjaxServlet?"
        ...
        success: function (data) {
            if (loadtable === 'true') {
                createTable(data, sexFlag, SearchType);
            }
...
function getLoad() {
    //alert(status);
    var URL = "_0_0_0_1_0_7_Dash_EmpCount_FacultyAndOrg_Ajax_1_getLoad_1_AjaxServlet?";
